...
## Graphical interface
 ...
 A simple Tkinter GUI is provided in `doomsday_gui.py`. Run `python doomsday_gui.py` and use the weekday buttons to make your guess, or press **Skip** or **Give Up**. After each guess the weekday buttons disappear and a **Next** button appears in the same spot so the window size stays constant. The interface keeps a running tally of correct and incorrect answers. A **Hint** button reveals the century's doomsday on the first click and the year's doomsday on the second.
